...
import (
	...
	v1alpha1 "github.com/imjasonh/gcb-task/pkg/apis/cloudbuild/v1alpha1"
	versioned "github.com/imjasonh/gcb-task/pkg/client/cloudbuild/clientset/versioned"
	cloudbuildv1alpha1 "github.com/imjasonh/gcb-task/pkg/client/cloudbuild/listers/cloudbuild/v1alpha1"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha1.Build.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Build. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Build) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Build.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Build. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Build) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.Build if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Build.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Build) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Build if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.Build.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha1.Build) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.Build resources.
type reconcilerImpl struct {
	...
	Lister cloudbuildv1alpha1.BuildLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister cloudbuildv1alpha1.BuildLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Builds(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.Build, desired *v1alpha1.Build) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.CloudbuildV1alpha1().Builds(desired.Namespace)
		...
		updater := r.Client.CloudbuildV1alpha1().Builds(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Build) (*v1alpha1.Build, error) {

	getter := r.Lister.Builds(resource.Namespace)
	...
	patcher := r.Client.CloudbuildV1alpha1().Builds(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Build) (*v1alpha1.Build, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Build, reconcileEvent reconciler.Event) (*v1alpha1.Build, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
